...
import { LASDecoder, LASFile } from "./workers/laslaz";
...
export class SceneLoader {
    ...
    async fetchAndProcessLASorLAZ(url: string): Promise<{ points: Float32Array, colors: Float32Array, bounds: Bounds }> {
        try {
            ...
            if (cachedData) {
                ...
                const bounds = this.calculateBounds(cachedData.points);
            ...
            const { points: rawPoints, colors: rawColors } = await this.loadFromFile(url);
            const { points, colors, bounds } = this.sortPointCloud(rawPoints, rawColors);

            this.storeInCache(url, points, colors).catch(err => {
    ...
    private async loadFromCache(url: string): Promise<{ points: Float32Array; colors: Float32Array } | null> {
    ...
    private async loadFromFile(url: string): Promise<{ points: Float32Array, colors: Float32Array }> {
    ...
    private sortPointCloud(points: Float32Array, colors: Float32Array): { points: Float32Array, colors: Float32Array, bounds: Bounds } {
        const bounds = this.calculateBounds(points);
        const sorter = new MortonSorter();
        const { sortedPoints, sortedIndices } = sorter.sort(points, bounds);
        const sortedColors = this.reorderFromSortedIndices(colors, sortedIndices);
        return { points: sortedPoints, colors: sortedColors, bounds };
    }

    async createQuadtree(points: Float32Array, bounds: Bounds): Promise<QuadTree> {
    ...
    private calculateBounds(points: Float32Array): Bounds {
    const pointStride = 4;
    ...
    private reorderFromSortedIndices(arr: Float32Array, sortedIndices: Uint32Array): Float32Array {
    const colorSize = 4;
